...
import { jaccardSimilarity } from '@timps-ai/memory-core';

export interface ToolContext {
  memoryClient: any;
  localEngine: any;
  timpsAPI: (path: string, method?: string, body?: unknown) => Promise<any>;
  chat: (message: string, username?: string) => Promise<string>;
  SERVER_MODE: boolean;
  TIMPS_USER_ID: number;
}
...
export function registerAllTools(
  ...
  const { memoryClient, localEngine, timpsAPI, chat, SERVER_MODE, TIMPS_USER_ID } = ctx;
