...
const nexusExplorer_1 = require("./nexusExplorer");
const synapseExplorer_1 = require("./synapseExplorer");
// ─── Global State ─────────────────────────────────────────────────────────────
let chatProvider;
// ─── Extension Activate ───────────────────────────────────────────────────────
let chronosVeilApiBase;
// ─── ChronosVeil integration ─────────────────────────────────────────────────
async function ingestToChronosVeil(content, sourceModule, tags, entity) {
    if (!chronosVeilApiBase) {
        const cfg = vscode.workspace.getConfiguration('timps');
        const userId = cfg.get('userId', 1);
        chronosVeilApiBase = cfg.get('apiBase', 'http://localhost:3000') + '/api';
    }
    try {
...
function activate(context) {
    ...
    const cfg = vscode.workspace.getConfiguration('timps');
    const userId = cfg.get('userId', 1);
    const apiBase = cfg.get('apiBase', 'http://localhost:3000');
    const nexusExplorer = new nexusExplorer_1.NexusForgeExplorerProvider(context.extensionUri, userId, apiBase);
    const synapseExplorer = new synapseExplorer_1.SynapseMetabolonExplorerProvider(context.extensionUri, userId, apiBase);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('timps.chatView', chatProvider, {
        webviewOptions: { retainContextWhenHidden: true }
    }));
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('timps.nexusForgeExplorer', nexusExplorer, {
        webviewOptions: { retainContextWhenHidden: true }
    }));
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('timps.synapseMetabolonExplorer', synapseExplorer, {
        webviewOptions: { retainContextWhenHidden: true }
    }));
    // Commands
    context.subscriptions.push(vscode.commands.registerCommand('timps.openChat', () => {
...
function deactivate() { }
// ─── Chat View Provider ───────────────────────────────────────────────────────
class TIMPSChatViewProvider {
    constructor(_context) {
        this._context = _context;
        this._sessions = [];
        this._currentSessionId = this._createSession();
        this._sessions = this._context.globalState.get('timps.sessions', []);
    ...
    _saveToChronosVeil(session, userMsg, assistantMsg) {
    ...
    _saveToSynapseMetabolon(userMsg, assistantMsg) {
        const cfg = vscode.workspace.getConfiguration('timps');
        const apiBase = cfg.get('apiBase', 'http://localhost:3000');
        const userId = cfg.get('userId', 1);
        const content = `User: ${userMsg.slice(0, 200)}\n\nTIMPS: ${assistantMsg.slice(0, 300)}`;
    ...
    _saveToNexusForge(userMsg, assistantMsg) {
        const cfg = vscode.workspace.getConfiguration('timps');
        const apiBase = cfg.get('apiBase', 'http://localhost:3000');
        const userId = cfg.get('userId', 1);
        const content = `User: ${userMsg.slice(0, 200)}\n\nTIMPS: ${assistantMsg.slice(0, 300)}`;
    ...
    async _handleUserMessage(content, sessionId) {
        ...
        try {
            await this._streamResponse(config, history, assistantMsg.id, (chunk) => {
    ...
    async _streamResponse(config, messages, _messageId, onChunk) {
        const allMessages = [
    ...
    _streamOllama(config, messages, onChunk) {
    ...
    _streamOpenAI(config, messages, onChunk) {
        return new Promise((resolve, reject) => {
            const body = JSON.stringify({
    ...
    _streamGemini(config, messages, onChunk) {
        return new Promise((resolve, reject) => {
