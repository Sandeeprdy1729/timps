...
// ─── HTTP helper ──────────────────────────────────────────────────────────────
function timpsRequest(path, method, body) {
...
async function timpsChat(message) {
...
async function analyzeDocument(doc) {
    ...
    if (!enableInlineWarnings)
        return;
    const ext = doc.languageId;
    if (!['typescript', 'javascript', 'python', 'java', 'go', 'rust'].includes(ext))
        return;
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            if (res?.verdict === 'CONTRADICTION' || res?.verdict === 'PARTIAL') {
                ...
                vscode.window.showWarningMessage(`⚠️ TIMPs: ${score}% contradiction! You previously stated: "${claim.slice(0, 80)}"`, 'Dismiss');
            }
            else {
...
async function cmdLookupAPI() {
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            if (response) {
                const panel = vscode.window.createWebviewPanel('timpsAPIQuirks', `TIMPs: ${apiName} Quirks`, vscode.ViewColumn.Beside, {});
                panel.webview.html = getWebviewContent(`API Archaeologist — ${apiName}`, response);
            }
            else {
...
async function cmdShowMemories() {
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            const panel = vscode.window.createWebviewPanel('timpsMemories', 'TIMPs: My Memories', vscode.ViewColumn.Beside, {});
            const content = memories.slice(0, 20).map((m) => `**[${m.memory_type?.toUpperCase()}]** (★${m.importance}) ${m.content}`).join('\n\n---\n\n');
...
// ─── Webview helper ───────────────────────────────────────────────────────────
function getWebviewContent(title, content) {
...
function activate(context) {
    ...
    // Commands
    context.subscriptions.push(vscode.commands.registerCommand('timps.checkContradiction', cmdCheckContradiction), vscode.commands.registerCommand('timps.lookupAPI', cmdLookupAPI), vscode.commands.registerCommand('timps.checkBugPattern', cmdCheckBugPattern), vscode.commands.registerCommand('timps.openDashboard', cmdOpenDashboard), vscode.commands.registerCommand('timps.showMemories', cmdShowMemories));
    // Inline warnings on file open
    context.subscriptions.push(vscode.workspace.onDidOpenTextDocument(doc => {
        if (getConfig().enableInlineWarnings)
            analyzeDocument(doc);
    }));
