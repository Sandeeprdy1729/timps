...
exports.processTerminalMessage = processTerminalMessage;
 ...
 */
function showBanner(terminal, model) {
    const lines = [
        '',
        color('  ╔══════════════════════════════════════╗', BOLD, TEAL),
...
function openTerminal() {
    ...
    const model = config.get('localModel', 'sandeeprdy1729/timps-coder');
 ...
 */
async function processTerminalMessage(text) {
    if (isProcessing) {
        return;
    }
    if (!activeTerminal) {
    ...
    try {
        ...
        const systemPrompt = (0, systemPrompt_1.getSystemPrompt)(customPrompt || undefined);
        ...
        for await (const chunk of (0, ollama_1.chat)(ollamaUrl, model, messages, { temperature, maxTokens }, abortController.signal)) {
            fullResponse += chunk;
            buffer += chunk;
