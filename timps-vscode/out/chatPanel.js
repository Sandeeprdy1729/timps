...
class TIMPsChatPanel {
    // ── Static factory ────────────────────────────────────────
    static createOrShow(extensionUri, memory) {
        if (currentPanel) {
            currentPanel._panel.reveal(vscode.ViewColumn.Beside);
            return currentPanel;
        }
        const panel = vscode.window.createWebviewPanel(TIMPsChatPanel.viewType, 'TIMPS Chat', vscode.ViewColumn.Beside, { enableScripts: true, retainContextWhenHidden: true, localResourceRoots: [extensionUri] });
        currentPanel = new TIMPsChatPanel(panel, extensionUri, memory);
        return currentPanel;
    }
    // Static helpers so extension.ts can post without holding a ref
    static postMemoryAudit(memories) {
        currentPanel?._panel.webview.postMessage({ command: 'memoryAudit', memories });
    }
    static postBlameResults(keyword, results) {
        currentPanel?._panel.webview.postMessage({ command: 'blameResults', keyword, results });
    }
    // ── Constructor ───────────────────────────────────────────
    constructor(panel, extensionUri, memory) {
        this._disposables = [];
        this._messages = [];
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.command) {
                ...
                case 'newSession':
                    ...
                    break;
                case 'applyCode':
                    await this._applyToEditor(msg.code);
                    break;
                case 'copyCode':
    ...
    async _handleMessage(text) {
        if (this._isStreaming || !text.trim())
            return;
        // Memory commands
        if (text.trim() === '!audit') {
        ...
        try {
            ...
            const systemPrompt = (0, systemPrompt_1.getSystemPrompt)(memCtx);
            const apiMessages = [
                { role: 'system', content: systemPrompt },
                ...this._messages
            ];
            let fullResponse = '';
            for await (const chunk of (0, ollama_1.chat)(ollamaUrl, model, apiMessages, { temperature, maxTokens }, this._abortController.signal)) {
                fullResponse += chunk;
    ...
    async _applyToEditor(code) {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('No active editor to apply changes.');
            return;
        }
    ...
    _post(data) { this._panel.webview.postMessage(data); }
    dispose() {
        currentPanel = undefined;
        this._panel.dispose();
...
TIMPsChatPanel.viewType = 'timpsChat';
function getNonce() {
    let t = '';
