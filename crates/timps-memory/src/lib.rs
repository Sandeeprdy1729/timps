...
impl MemoryStore {
    ...
    pub fn open(project_path: &str) -> Result<Self> {
        ...
        Ok(Self {
            ...
            semantic_lock: Arc::new(Mutex::new(())),
        })
    }
    ...
    pub async fn load_semantic(&self) -> Result<Vec<SemanticEntry>> {
        ...
        serde_json::from_str(&content).map_err(|e| {
            // Back up corrupt file so the user doesn't lose data entirely
            let backup = self.dir.join(format!("semantic.json.corrupt.{}", chrono::Utc::now().timestamp()));
            let _ = std::fs::copy(&path, &backup);
            anyhow::anyhow!("corrupt semantic.json (backed up to {}): {e}", backup.display())
        })
    }

    pub async fn save_semantic(&self, entries: &[SemanticEntry]) -> Result<()> {
        let path = self.dir.join("semantic.json");
        let tmp = self.dir.join("semantic.json.tmp");
    ...
    pub async fn store_semantic(&self, entry: SemanticEntry) -> Result<()> {
        let _guard = self.semantic_lock.lock().await;
