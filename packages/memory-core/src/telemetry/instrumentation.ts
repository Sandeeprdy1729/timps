...
export function instrumentBackend(
  ...
  const wrapAsync = <T>(method: string, fn: (...args: any[]) => T | Promise<T>): (...args: any[]) => T | Promise<T> => {
    return (...args: any[]): T | Promise<T> => {
      ...
      try {
        ...
        if (result instanceof Promise) {
          return result.then(
            (res) => { finishOk(res); return res; },
            (err) => { finishErr(err); throw err; },
          ) as T | Promise<T>;
        }
        finishOk(result);
        return result;
