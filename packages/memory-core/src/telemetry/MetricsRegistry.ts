...
export class MetricsRegistry {
   ...
   * Sanitizes metric names (dots → underscores), appends _total for counters,
   * deduplicates TYPE headers, and follows Prometheus naming conventions.
   */
  prometheusExport(): string {
    ...
    for (const [name, entries] of this._counters) {
      ...
      for (const e of entries) {
        ...
        lines.push(`${sName}_total${labels} ${e.value} ${now}`);
      }
    }

    for (const [name, entries] of this._histograms) {
      const sName = sanitize(name);
      lines.push(`# TYPE ${sName} histogram`);
      for (const e of entries) {
        const attrs = Object.entries(e.attributes)
          .map(([k, v]) => `${k}="${v}"`)
          .join(',');
