...
import { isPromiseLike } from './migrations/async.js';
...
export class MemoryEngine {
  ...
  private _runMigrations(): void {
    ...
    if (engine.hasPending) {
      ...
      // Async backends (Postgres/Redis/Qdrant) migrate in the background —
      // the constructor can't await, so surface failures via the console
      // instead of crashing startup with a thrown "not iterable" error.
      if (isPromiseLike(result)) {
        void (result as Promise<void>).catch((err: unknown) => {
          console.error('[memory-core] migration startup failed:', err);
        });
      }
    }
  }
  ...
  /** CRDT conflict/merge/check metric recorders (no-op when telemetry is off). */
  get crdtMetrics(): { recordConflict: (r: string) => void; recordMerge: (d: number) => void; recordCheck: () => void } {
    return this._crdtMetrics;
  }
  ...
  store(entry: { content: string; type?: MemoryEntryType; tags?: string[]; platform?: string; channel?: string; timestamp?: number }, opts?: { skipGuard?: boolean }): void {
    ...
    this._crdtMetrics.recordCheck();
    const { content, type = 'fact', tags = [], platform, channel, timestamp: explicitTimestamp } = entry;
    if (facts.some(f => jaccardSimilarity(f.content, content) > 0.8)) {
      this._crdtMetrics.recordConflict('dedup');
      return;
    }
    ...
    let evidenceCount = 1;
    if (!opts?.skipGuard) {
      // NEW — ask ChronosForge whether this content conflicts with an
      // already-established, more-reliable fact before the guard decides.
      // Domain defaults to 'general' here since inferDomain() isn't exported
      // from ChronosForge; revisit if per-domain routing matters later.
      const chronosConflict = this.chronosForge.checkConflict(content, 'general', confidence);
      if (chronosConflict) this._crdtMetrics.recordConflict('chronos');
      ...
      } as never, 0, chronosConflict);
      if (!guardVerdict.allowed) {
        this._crdtMetrics.recordConflict('guard_rejected');
    ...
    // ── Commit ordering ──
    // ALL side-effect writes (forge weaves, engram log, provenance, context
    // vector) happen BEFORE the semantic store persists. The semantic store is
    // the atomic commit point: if any earlier layer write fails (disk full,
    // backend error), store() aborts and the fact is NOT committed — avoiding
    ...
    // Semantic commit point — persisted LAST so a failed side-effect write
    // aborts the whole store() with no partial commit.
    facts.push({ id, timestamp, type, content, tags: enrichedTags });
    saveSemantic(this.dir, facts, this._backend);
    this._telemetry?.metrics.gauge('memory.semantic_entries', facts.length);
  ...
  /**
   * Build the engine-level cascade cache key for a recall query. Shared by
   * recall() (read) and warmupCache() (write) so warmed entries are actually
   * hit — previously warmup hardcoded `recall:{q}:fact:5:0:1` while recall
  ...
  private _recallCacheKey(query: string, options?: SearchOptions): string {
    ...
    return `${projectPrefix}:recall:${query.toLowerCase().trim()}:${optSuffix}`;
  }
  ...
  async recall(query: string, options?: SearchOptions): Promise<ScoredMemoryEntry[]> {
    ...
    try {
      ...
      if (useCache) {
        const cacheKey = this._recallCacheKey(query, options);
  ...
  async getContextString(task = ''): Promise<string> {
    ...
    // predict() is in-memory reservoir compute (no I/O), so awaiting it is
    // cheap; the .then()-push approach was dead code because parts.join() ran
    // before the microtask, dropping the burnout warning from every context.
    try {
      const burnoutPred = await this.echoForge.predict('burnout');
      if (burnoutPred.riskLevel === 'high') {
        parts.push(`\u26a0\ufe0f Echo burnout signal (${(burnoutPred.riskScore * 100).toFixed(0)}%): ${burnoutPred.explanation}`);
      }
  ...
  consolidate(): number {
    ...
    return removed;
      ...
      const duration = Date.now() - startTime;
      this._crdtMetrics.recordMerge(duration);
      if (this._telemetry) {
  ...
  async warmupCache(commonQueries: string[] = ['context', 'current', 'active', 'pattern', 'recent']): Promise<void> {
    ...
    for (const q of commonQueries) {
      try {
        const warmOpts = { limit: 5, useCache: false };
        const results = await this._recallCompute(q, warmOpts);
        if (results.length > 0) {
          const cacheKey = this._recallCacheKey(q, warmOpts);
