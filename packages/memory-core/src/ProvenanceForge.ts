...
export class ProvenanceForge {
  ...
  constructor(private dir: string, backend?: StorageBackend) {
    ...
    this.provDir = path.join(this.dir, 'provenance');
    if (!backend) fs.mkdirSync(this.provDir, { recursive: true });
  }

  private key(id: string): string {
    return `provenance/${id}.json`;
  }
  ...
  record(input: ProvenanceInput, overrideId?: string): Provenance {
    ...
    if (this._backend) {
      this._backend.write(this.key(hash), full);
    } else {
      const file = path.join(this.provDir, `${hash}.json`);
  ...
  explain(id: string): Provenance | null {
    if (this._backend) {
      return (this._backend.read(this.key(id)) as Provenance | null) ?? null;
    }
    const file = path.join(this.provDir, `${id}.json`);
  ...
  addCustodyStep(id: string, actor: string, op: string): Provenance | null {
    ...
    if (this._backend) {
      this._backend.write(this.key(id), existing);
    } else {
      const file = path.join(this.provDir, `${id}.json`);
  ...
  delete(id: string): boolean {
    if (this._backend) {
      const before = this.explain(id);
      this._backend.delete(this.key(id));
      return before !== null;
    }
    const file = path.join(this.provDir, `${id}.json`);
  ...
  listBySource(kind: SourceKind): Provenance[] {
    ...
    if (this._backend) {
      ...
      for (const f of jsonFiles) {
        ...
        if (p && p.sourceKind === kind) out.push(p);
      }
      return out;
    }
    const files = fs.readdirSync(this.provDir).filter(f => f.endsWith('.json'));
    for (const f of files) {
      const p = JSON.parse(fs.readFileSync(path.join(this.provDir, f), 'utf-8')) as Provenance;
