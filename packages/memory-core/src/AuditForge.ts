...
export class AuditForge {
  ...
  private auditWorking(): AuditSection {
    const data = this._readJSON('working.json', null);
    if (data === null) return { weak: 0, contradicted: 0, outdated: 0, unsourced: 0, entries: [] };
  ...
  private auditEpisodic(): AuditSection {
    ...
    if (!Array.isArray(episodes) || episodes.length === 0) {
      return { weak: 0, contradicted: 0, outdated: 0, unsourced: 0, entries: [] };
    }
    const entries: AuditSection['entries'] = [];
    let outdated = 0;
  ...
  private auditSemantic(): AuditSection {
    const data = this._readJSON('semantic.json', []);
    ...
    for (const f of this._listProvenanceFiles()) {
      try {
        const p = this._backend
          ? (this._backend.read(f) as any)
          : JSON.parse(fs.readFileSync(f, 'utf-8'));
        if (!p) continue;
        if (p.id) sourced.add(p.id);
        if (Array.isArray(p.parentIds)) for (const pid of p.parentIds) sourced.add(pid);
      } catch { /* skip malformed */ }
    }
    const unsourced = data.filter((e: any) => e && !sourced.has(e.id)).length;
  ...
  /** Enumerate provenance JSON files (backend keys or fs paths). */
  private _listProvenanceFiles(): string[] {
    if (this._backend) {
      const files = this._backend.list('provenance/') ?? [];
      return (Array.isArray(files) ? files : []).filter((f: string) => f.endsWith('.json'));
    }
    const provDir = path.join(this.dir, 'provenance');
    if (!fs.existsSync(provDir)) return [];
    return fs.readdirSync(provDir)
      .filter(f => f.endsWith('.json'))
      .map(f => path.join(provDir, f));
  }
