...
describe('AetherForgeERL — weave', () => {
  ...
  it('populates joinResult when weaving near-identical content (M43 regression)', () => {
    // Pre-M43, weave() called join()/meet() BEFORE inserting the new node into
    // storeData.nodes, so join() always returned null and joinResult was dead.
    forge.weave('Postgres is the right database choice', { domain: 'decision', evidenceCount: 5 });
    const result = forge.weave('Postgres is the right database choice', { domain: 'decision', evidenceCount: 5 });
    expect(result.joinResult).toBeDefined();
    expect(result.joinResult!.survivorId).toBe(result.nodeId);
    expect(result.joinResult!.absorbedId).toBe(result.supersededIds[0]);
  });

  it('weave() meet resolves detected contradictions (M43 regression)', () => {
    // Pre-M43 the new node was missing from storeData.nodes when meet() ran,
    // so meet() early-returned with empty resolution and the contradiction was
    // never resolved during weave.
    forge.weave('Use Redis for caching is great', { domain: 'decision', evidenceCount: 5 });
    const result = forge.weave('Never use Redis for caching', { domain: 'decision', evidenceCount: 5 });
    expect(result.detectedContradictions.length).toBeGreaterThanOrEqual(1);
    expect(result.meetResult).toBeDefined();
    expect(result.meetResult!.resolutionEdgeIds.length).toBeGreaterThanOrEqual(1);
  });
});
