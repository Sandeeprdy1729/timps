// ── @timps/memory-core — Phase 4c: ClusterEngine ──
...
export class ClusterEngine {
  ...
  private _kMeansCluster(memories: ClassifiedMemory[]): MemoryCluster[] {
    ...
    centroids.push(memories[firstIdx].embedding!.slice(0, dim));

    // Remaining: weighted by distance-squared
    for (let c = 1; c < k; c++) {
      const distSqs: number[] = [];
      for (let i = 0; i < memories.length; i++) {
        if (indices.has(i)) { distSqs.push(0); continue; }
        const emb = memories[i].embedding!.slice(0, dim);
        const minDist = Math.min(...centroids.map(cen => this._cosineDist(emb, cen)));
        distSqs.push(minDist * minDist);
      }
      ...
      indices.add(pick);
      centroids.push(memories[pick].embedding!.slice(0, dim));
    }

    // Iterate
    const assignments = new Array(memories.length).fill(0);
