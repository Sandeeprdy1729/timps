...
// Asserts the honest (M58) behavior: deterministic content embeddings,
// content-based contradiction detection, and manifold-proximity query.
...
// Content that genuinely contradicts: same vocabulary, opposing stance.
const PRO_STATEMENT = 'Use PostgreSQL for database storage';
const CON_STATEMENT = 'Do not use PostgreSQL for database storage';

describe('QPTW', () => {
  describe('weave', () => {
    it('creates a node with deterministic content-derived phase', () => {
      const q = new QPTW(tmpDir());
      const r = q.weave(PRO_STATEMENT, { domain: 'decision' });
    ...
    it('creates causal edge when parentId provided', () => {
      const q = new QPTW(tmpDir());
      const parent = q.weave(PRO_STATEMENT, { domain: 'decision' });
    ...
    it('creates contradicts edges for opposing-stance same-domain nodes', () => {
      const q = new QPTW(tmpDir());
      q.weave(PRO_STATEMENT, { domain: 'decision' });
      q.weave(CON_STATEMENT, { domain: 'decision' });
      const edges = q.exportEdges();
      const contraEdges = edges.filter(e => e.edgeType === 'contradicts');
      expect(contraEdges.length).toBe(1);
    });

    it('does not create contradicts edges for neutral content', () => {
      const q = new QPTW(tmpDir());
      q.weave('PostgreSQL is a relational database', { domain: 'decision' });
      q.weave('MongoDB is a document database', { domain: 'decision' });
      const edges = q.exportEdges();
      expect(edges.filter(e => e.edgeType === 'contradicts')).toHaveLength(0);
    });
  ...
  describe('updateAffected', () => {
    ...
    it('propagates to neighbors via causal fanout', () => {
      const q = new QPTW(tmpDir());
  ...
  describe('detectContradictions', () => {
    ...
    it('finds contradictions between opposing-stance nodes in same domain', () => {
      const q = new QPTW(tmpDir());
      q.weave(PRO_STATEMENT, { domain: 'decision' });
      q.weave(CON_STATEMENT, { domain: 'decision' });
