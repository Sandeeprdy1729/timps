...
describe('ProvenanceForge — L13: source tracking chain', () => {
  ...
  it('record creates a JSON file on disk', () => {
    ...
    const file = path.join(dir, 'provenance', `${p.id}.json`);
  ...
  it('delete removes the file', () => {
    ...
    expect(fs.existsSync(path.join(dir, 'provenance', `${p.id}.json`))).toBe(false);
  });
...
// ────────────────────────────────────────────────────────────────────
// Regression (M44): provenance path contract
// ProvenanceForge must write under {dir}/provenance/ so that AuditForge,
// BiasRevealer, SourceAttributor and MemoryAuditor all find records.
// ────────────────────────────────────────────────────────────────────
describe('Provenance path contract (M44) — all consumers agree', () => {
  let dir: string;
  ...
  it('ProvenanceForge writes records into a provenance/ subdir', () => {
    const forge = new ProvenanceForge(dir);
    const p = forge.record({
      sourceKind: 'user_direct',
      sourceDetail: 'typed',
      actorId: 'u1',
    ...
    expect(fs.existsSync(path.join(dir, 'provenance', `${p.id}.json`))).toBe(true);
    expect(fs.existsSync(path.join(dir, `${p.id}.json`))).toBe(false);
  });

  it('AuditForge.auditSemantic counts 0 unsourced after stores with provenance', () => {
    const forge = new ProvenanceForge(dir);
    const id = 'mem-m44';
    fs.writeFileSync(path.join(dir, 'semantic.json'), JSON.stringify([
      { id, content: 'sourced fact', timestamp: Date.now() },
      { id: 'mem-noprov', content: 'no provenance', timestamp: Date.now() },
    ]), 'utf-8');
    forge.record({
    ...
    const report = new AuditForge(dir).run();
    expect(report.semantic.unsourced).toBe(1);
  });

  it('SourceAttributor resolves a memory stored via ProvenanceForge', () => {
    const forge = new ProvenanceForge(dir);
    const id = 'mem-attrib';
    forge.record({
      sourceKind: 'agent_inference',
    ...
    const attr = new SourceAttributor(dir).attribute(id);
    expect(attr).not.toBeNull();
    expect(attr!.sourceKind).toBe('agent_inference');
  });

  it('BiasRevealer.sourceBias is populated from ProvenanceForge records', () => {
    const forge = new ProvenanceForge(dir);
    for (let i = 0; i < 3; i++) {
      const id = `mem-${i}`;
      forge.record({
    ...
    fs.writeFileSync(path.join(dir, 'semantic.json'), JSON.stringify([
      { id: 'mem-0', content: 'good result', tags: [], type: 'fact', timestamp: Date.now() },
      { id: 'mem-1', content: 'good result', tags: [], type: 'fact', timestamp: Date.now() },
      { id: 'mem-2', content: 'good result', tags: [], type: 'fact', timestamp: Date.now() },
    ]), 'utf-8');
    const reveal = new BiasRevealer(dir).reveal();
    expect(reveal.sourceBias.length).toBeGreaterThan(0);
    expect(reveal.sourceBias[0].sourceKind).toBe('user_direct');
  });

  it('MemoryAuditor reports these memories as sourced', async () => {
    const forge = new ProvenanceForge(dir);
    const id = 'mem-audited';
    fs.writeFileSync(path.join(dir, 'semantic.json'), JSON.stringify([
      { id, content: 'fact', confidence: 0.9, timestamp: Date.now() },
    ]), 'utf-8');
    forge.record({
      sourceKind: 'user_direct',
      sourceDetail: 'typed',
    ...
    const report = await new MemoryAuditor(dir).audit();
...
// ────────────────────────────────────────────────────────────────────
// Regression (M53): audit must be sub-quadratic + wrappers forward backend
// 1) MemoryAuditor no longer instantiates a ConflictResolver per audit and no
//    longer scans every O(n²) pair. It indexes entries by significant token and
//    evaluates only pairs that SHARE a token — exact, since disjoint pairs have
//    jaccard 0 (< 0.2) and can never conflict. It also uses the pure
//    ConflictResolver.evaluate so audits never write per-pair resolution files.
// 2) The Tool 19 (ConfidenceCalibratorTool) and Tool 23 (ProspectiveTriggerTool)
//    wrappers now forward their StorageBackend into the underlying L22/L17 layer
//    so persistence routes through the backend, not raw fs writes.
// ────────────────────────────────────────────────────────────────────
describe('M53 — audit sub-quadratic + wrapper backend forwarding', () => {
  let dir: string;

  beforeEach(() => { dir = tmpDir(); });
  afterEach(() => fs.rmSync(dir, { recursive: true, force: true }));

  it('audit detects a conflicting pair sharing tokens', async () => {
    fs.writeFileSync(path.join(dir, 'semantic.json'), JSON.stringify([
      { id: 's1', content: 'the team should keep the old database schema', confidence: 0.8, timestamp: Date.now() },
      { id: 's2', content: 'the team must remove the old database schema', confidence: 0.9, timestamp: Date.now() },
    ]), 'utf-8');
    const report = await new MemoryAuditor(dir).audit();
    expect(report.contradicted).toBeGreaterThan(0);
  });

  it('audit does not write a conflict-resolutions file', async () => {
    fs.writeFileSync(path.join(dir, 'semantic.json'), JSON.stringify([
      { id: 's1', content: 'the team should keep the old database schema', confidence: 0.8, timestamp: Date.now() },
      { id: 's2', content: 'the team must remove the old database schema', confidence: 0.9, timestamp: Date.now() },
    ]), 'utf-8');
    await new MemoryAuditor(dir).audit();
    expect(fs.existsSync(path.join(dir, 'conflict-resolutions.json'))).toBe(false);
  });

  it('audit over 3000 entries sharing a common token completes fast', async () => {
    const entries = [];
    for (let i = 0; i < 3000; i++) {
      entries.push({ id: `s${i}`, content: `should the subsystem ${i} use this service`, confidence: 0.8, timestamp: Date.now() });
    }
    fs.writeFileSync(path.join(dir, 'semantic.json'), JSON.stringify(entries), 'utf-8');
    const start = Date.now();
    const report = await new MemoryAuditor(dir).audit();
    const elapsed = Date.now() - start;
    expect(report.totalEntries).toBe(3000);
    // The old O(n²) loop would evaluate ~4.5M pairs; the bucketed+capped path
    // evaluates at most 64-choose-2 per shared token. Generous bound to stay
    // CI-safe while still catching a regression to quadratic behavior.
    expect(elapsed).toBeLessThan(2000);
  });

  it('ProspectiveTriggerTool forwards backend for persistence', () => {
    const backend = new InMemoryBackend();
    const tool = new ProspectiveTriggerTool(dir, backend);
    const t = tool.register({ when: 'deploy', surface: 'run checklist', memoryId: 'm1' });
    const stored = backend.read('prospective/triggers.json');
    expect(Array.isArray(stored)).toBe(true);
    expect(stored[0].id).toBe(t.id);
    expect(fs.existsSync(path.join(dir, 'prospective-triggers.json'))).toBe(false);
    const tool2 = new ProspectiveTriggerTool(dir, backend);
    expect(tool2.list().length).toBe(1);
  });

  it('ConfidenceCalibratorTool forwards backend for persistence', () => {
    const backend = new InMemoryBackend();
    const tool = new ConfidenceCalibratorTool(dir, backend);
    tool.record('mem1', { similarity: 0.9, reliability: 0.8, evidence: 0.6, freshness: 1 });
    const stored = backend.read('confidence/calibrator.json');
    expect(Array.isArray(stored)).toBe(true);
    expect(stored[0].memoryId ?? stored[0].id).toBe('mem1');
  });
});
