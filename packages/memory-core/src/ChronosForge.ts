...
// NEW — threshold for wordJaccard(), calibrated by hand against this
// project's own recently_superseded eval cases (see eval/data/results-v2.json):
// 0.25 catches 2 of 3 known paraphrase-conflict pairs (scored 0.273, 0.308)
// while staying below unrelated-content pairs (scored 0.0, 0.143). The third
// known pair (0.167) shares too little vocabulary for ANY lexical method to
// catch — that one needs the embeddingSimilarityHook, not a lower threshold
// here (lowering it further would start flagging unrelated content).
const WORD_JACCARD_THRESHOLD = 0.25;
const STOPWORDS = new Set(['the', 'and', 'for', 'are', 'was', 'using', 'has', 'per', 'this', 'that', 'with', 'from', 'have']);

// NEW — word-level Jaccard on meaning-bearing tokens. Complements
// trigramJaccard: trigram catches near-identical PHRASING, this catches
// shared VOCABULARY even when sentence structure differs.
...
export class ChronosForge implements IMemoryLayer {
   ...
   *
   * The retrievalCount boost for returned nodes is applied in-memory only
   * (simulates Ebbinghaus boost during a session) and is NEVER flushed to
   * disk from here. Reads must not mutate or write the node store — that
   * keeps query paths lock-free and prevents a concurrent engine's read from
   * clobbering another engine's superseded/invalidAt updates (write-on-read).
   * The boost is persisted on the next real write (weave/verify/consolidate/
   * archive keep disk persistence).
   */
  queryAt(
    atTime: number,
    ...
    // In-memory-only retrieval boost — NO disk write. Keeps Ebbinghaus boost
    // within the session without write-on-read clobbering on shared backends.
    for (const n of valid) n.retrievalCount++;
  ...
  /**
   * NEW — checkConflict(): write-time staleness check for ConstitutionalGuard.
   *
   * Does not touch weave()'s behavior. Scans currently-VALID nodes in the same
   * domain for high textual overlap with `content`, using TWO signals:
   *   1. Character-trigram Jaccard (catches near-identical phrasing — the
   *      original signal, good for exact/close restatements).
   *   2. Word-level Jaccard on meaning-bearing tokens (catches paraphrases
   *      that share key vocabulary in a different sentence structure — the
   *      trigram check misses these, e.g. "described as a 22-layer
   *      architecture" vs "description... says... is 9-layer").
   * If EITHER signal crosses its threshold and the existing node is more
   * reliable than the incoming write, it's flagged as a conflict.
   *
   * Honest limitation, tested and confirmed on this project's own eval set:
   * word-Jaccard catches paraphrases that still share vocabulary (e.g. "160
   * of 160 swarm tools... correctly wired" vs "only 64 swarm tools... per the
   * old docs" shares almost no words despite meaning the same thing) — that
   * class of conflict needs real semantic embeddings, not lexical overlap.
   * See embeddingSimilarityHook below for the production path once real
   * embedding access (Ollama/OpenAI via EmbeddingService) is available.
   */
  checkConflict(
    content: string,
    domain: SignalDomain,
    incomingReliability: number,
    opts?: { embeddingSimilarityHook?: (a: string, b: string) => number; embeddingThreshold?: number }
  ): { conflict: boolean; conflictsWithNodeId?: string; overlap?: number; existingReliability?: number; signal?: 'trigram' | 'word-jaccard' | 'embedding' } {
    const now = nowMs();
    const nodes = this._loadNodes();
    const candidates = nodes
      .filter(n => n.domain === domain && n.invalidAt === null && (n.validTo === null || n.validTo > now))
    ...
    for (const cand of candidates) {
      if (cand.baseImportance <= incomingReliability) continue; // only conflicts with MORE reliable existing facts

      const trigramOverlap = trigramJaccard(content, cand.content);
      if (trigramOverlap >= CONTRADICTION_LOWER) {
        return { conflict: true, conflictsWithNodeId: cand.id, overlap: trigramOverlap, existingReliability: cand.baseImportance, signal: 'trigram' };
      }

      const wordOverlap = wordJaccard(content, cand.content);
      if (wordOverlap >= WORD_JACCARD_THRESHOLD) {
        return { conflict: true, conflictsWithNodeId: cand.id, overlap: wordOverlap, existingReliability: cand.baseImportance, signal: 'word-jaccard' };
      }

      if (opts?.embeddingSimilarityHook) {
        const embSim = opts.embeddingSimilarityHook(content, cand.content);
        const threshold = opts.embeddingThreshold ?? 0.75;
        if (embSim >= threshold) {
          return { conflict: true, conflictsWithNodeId: cand.id, overlap: embSim, existingReliability: cand.baseImportance, signal: 'embedding' };
        }
      }
    }
