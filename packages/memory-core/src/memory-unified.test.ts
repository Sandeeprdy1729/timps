...
describe('projectHash — canonical path hashing', () => {
  test('symlinked paths hash to the same project as their real path', () => {
    const real = path.join(os.tmpdir(), `timps-hash-${Date.now()}`);
    const link = path.join(os.tmpdir(), `timps-hash-link-${Date.now()}`);
  ...
  test('nonexistent paths still produce a stable resolved hash', () => {
    const a = projectHash('/definitely/not/a/real/dir');
    const b = projectHash('/definitely/not/a/real/dir');
    expect(a).toBe(b);
