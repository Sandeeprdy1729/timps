...
import type { StorageBackend, StorageQuery, StorageRecord, StorageTransaction } from '../backends/types.js';
...
// ── Async backend regression tests (M57) ──
// Postgres/Redis/Qdrant backends return Promises from read/write/list.
// Migrations must await async results instead of casting to string[].

/** Minimal async backend simulating Postgres/Redis — every op returns a Promise. */
class AsyncStubBackend implements StorageBackend {
  ...
  query(filter: StorageQuery): Promise<StorageRecord[]> {
    const prefix = filter.prefix ?? '';
    const records: StorageRecord[] = [];
    for (const [key, value] of this.store) {
      if (!key.startsWith(prefix)) continue;
      if (filter.filter && !filter.filter(value)) continue;
      records.push({ key, value });
    }
    return Promise.resolve(records);
  }

  beginTxn(): Promise<StorageTransaction> {
    return Promise.resolve({
      write: (key: string, value: any) => this.store.set(key, value),
...
describe('MigrationEngine on async backends (M57)', () => {
  it('migrates from version 0 without throwing "not iterable"', async () => {
    const backend = new AsyncStubBackend();
    backend.write('echo/echoforge.json', { version: '2.0', nodes: {} });

    const engine = new MigrationEngine('/async/dir', backend, ALL_MIGRATIONS);
    expect(engine.hasPending).toBe(true); // deferred — assumes pending
    ...
    // v2→v3 must have added _meta (would have thrown on a Promise list)
    const echo = await backend.read('echo/echoforge.json');
    expect(echo._meta).toBeDefined();
    expect(echo._meta.schemaVersion).toBe(3);
    expect(echo.version).toBe('2.0'); // original data preserved
  });

  it('runs only pending migrations when partially upgraded', async () => {
    const backend = new AsyncStubBackend();
    backend.write('schema-version.json', { version: 3, migratedAt: '2026-01-01' });
    backend.write('echo/echoforge.json', {
      _meta: { schemaVersion: 3, layerName: 'echo', createdAt: '2026-01-01', migratedAt: '2026-01-01' },
      version: '2.0',
    });
    ...
    expect(engine.currentVersion()).toBe(CURRENT_SCHEMA_VERSION);
    // Only v3→v4 ran — _meta already present, so only orgScope was added
    const echo = await backend.read('echo/echoforge.json');
    expect(echo._meta.orgScope).toBeDefined();
    expect(echo._meta.migratedAt).not.toBe('2026-01-01');
  });

  it('skips all migrations when already at latest version', async () => {
    const backend = new AsyncStubBackend();
    backend.write('schema-version.json', { version: CURRENT_SCHEMA_VERSION, migratedAt: '2026-01-01' });
    backend.write('echo/echoforge.json', {
      _meta: { schemaVersion: 3, layerName: 'echo', createdAt: '2026-01-01', migratedAt: '2026-01-01' },
      version: '2.0',
    });
    ...
    expect(engine.hasPending).toBe(false);
    const echo = await backend.read('echo/echoforge.json');
    expect(echo._meta.migratedAt).toBe('2026-01-01'); // untouched
  });
});
