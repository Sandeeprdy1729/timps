...
export class ConflictResolver {
  ...
  /**
   * Pure conflict computation — no instance state, no disk/backend I/O.
   * Used by read-only consumers (MemoryAuditor's scan over all semantic
   * entries) so an audit does not write one file per candidate pair.
   */
  static evaluate(a: MemoryRef, b: MemoryRef): ConflictResolution {
  ...
  resolve(a: MemoryRef, b: MemoryRef): ConflictResolution {
    const result = ConflictResolver.evaluate(a, b);
    // Persist only comparisons the original implementation recorded
    // (sim >= 0.2); unrelated pairs are not resolution history.
