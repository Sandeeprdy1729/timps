...
import { loadDataset, loadAllDatasets, createFreshEngine, seedEngineWithDataset, evaluateDataset, formatEvalSummary, computeDatasetSha, paraphraseSeed } from './runner.js';
...
import type { EvalResult, EvalBaseline, VariantConfig, RegressionResult, EvalDataset } from './types.js';
...
describe('Eval Self-Fulfillment Guard', () => {
  it('paraphraseSeed is deterministic and not a literal copy', () => {
    const src = 'Memory is the substrate for cross-referenced recall';
    const a = paraphraseSeed(src);
    const b = paraphraseSeed(src);
    expect(a).toBe(b);
    expect(a).not.toBe(src);
    expect(a).not.toContain(src);
  });

  it('paraphraseSeed preserves the content token bag (BM25 ranking intact)', () => {
    const src = 'PostgreSQL with read replicas is the primary database';
    const reworded = paraphraseSeed(src);
    const expectTokens = new Set(
      src.toLowerCase().split(/\s+/).filter(w => w.length > 2),
    );
    for (const t of expectTokens) {
      expect(reworded.toLowerCase()).toContain(t);
    }
  });

  it('seedEngineWithDataset no longer stores expectedMemories verbatim', () => {
    const dataset = loadDataset('multi-layer-recall');
    const engine = createFreshEngine();
    seedEngineWithDataset(engine, dataset);
    const stored = engine.getSemanticEntries().map(e => e.content);
    for (const ex of dataset.examples) {
      for (const em of ex.expectedMemories) {
        expect(stored).not.toContain(em);
      }
    }
  });

  it('flags self-fulfilling examples and excludes them from aggregates', async () => {
    const engine = createFreshEngine();
    const dataset: EvalDataset = {
      name: 'vacuity-check',
      version: '1.0.0',
      description: 'deliberately self-fulfilling dataset',
      examples: [
        { query: 'What port?', expectedMemories: ['The server runs on port 8080'] },
      ],
    };
    engine.store({ content: 'The server runs on port 8080', type: 'fact', tags: ['vacuity-check'] });
    const result = await evaluateDataset(engine, dataset, 'run', 'abc', 'default');

    expect(result.perExample[0].trivial).toBe(true);
    expect(result.summary.trivialExcluded).toBe(1);
    expect(result.summary.passed).toBe(0);
    expect(result.summary.avgRecallAt5).toBe(0);
  });

  it('scores paraphrase-seeded recall without self-fulfillment', async () => {
    const dataset = loadDataset('multi-layer-recall');
    const engine = createFreshEngine();
    seedEngineWithDataset(engine, dataset);
    const result = await evaluateDataset(engine, dataset, 'test-run', 'abc123', 'default');

    expect(result.summary.trivialExcluded).toBe(0);
    for (const pe of result.perExample) {
      expect(pe.trivial).toBeUndefined();
    }
    expect(result.summary.avgRecallAt5).toBeGreaterThan(0);
  });
});
...
describe('BaselineManager', () => {
  ...
  it('saves and loads a baseline', () => {
    const mockResult: EvalResult = {
      ...
      perExample: [],
      summary: { totalExamples: 8, passed: 7, failed: 1, avgRecallAt5: 87.5, avgRecallAt10: 95.0, avgMrr: 0.85, avgLatencyMs: 30, trivialExcluded: 0, p50LatencyMs: 25, p95LatencyMs: 45, p99LatencyMs: 60 },
    };
...
describe('RegressionDetector', () => {
  it('detects regression when below threshold', () => {
    ...
    const result: EvalResult = {
      ...
      perExample: [],
      summary: { totalExamples: 8, passed: 6, failed: 2, avgRecallAt5: 75, avgRecallAt10: 85, avgMrr: 0.7, avgLatencyMs: 30, trivialExcluded: 0, p50LatencyMs: 25, p95LatencyMs: 45, p99LatencyMs: 60 },
    };
  ...
  it('passes when metric meets threshold', () => {
    ...
    const result: EvalResult = {
      ...
      perExample: [],
      summary: { totalExamples: 8, passed: 8, failed: 0, avgRecallAt5: 100, avgRecallAt10: 100, avgMrr: 1.0, avgLatencyMs: 30, trivialExcluded: 0, p50LatencyMs: 25, p95LatencyMs: 45, p99LatencyMs: 60 },
    };
