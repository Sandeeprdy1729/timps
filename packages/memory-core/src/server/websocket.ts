...
export class MemoryWsServer {
  ...
  constructor(server: any, engine: MemoryEngine, path = '/ws', apiKeys?: Map<string, ApiKeyRecord>) {
    this.engine = engine;
    this.apiKeys = apiKeys ?? new Map();
  ...
  private handleConnection(ws: WebSocket, req: IncomingMessage): void {
    ...
    if (apiKey && apiKey.startsWith('timps_')) {
      // API key auth: extract key ID and verify hash
      const keyId = apiKey.slice(6, 14); // timps_XXXXXXXX...
      const record = this.apiKeys.get(keyId);
      if (!record || !verifyApiKey(apiKey, record.hash)) {
      ...
      ws.close(4001, 'Unauthorized: provide ?key=<api_key> or ?token=<jwt>');
      return;
    }
