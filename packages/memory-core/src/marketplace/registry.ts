...
export class PluginRegistry {
  ...
  submit(pkg: PluginPackage): SubmissionResult {
    ...
    const blocked = !approved(scanResults);

    // Static analysis is a regex ADVISORY, not proof of safety: obfuscated JS
    // (e.g. 'req' + 'uire') trivially evades literal patterns, so JS plugins
    // are never auto-approved — they queue for human review. WASM plugins run
    // inside the sandbox, so they may auto-approve when checks pass.
    const isApproved = !blocked && pkg.format === 'wasm';
    const status = blocked ? 'rejected' : (isApproved ? 'approved' : 'pending_review');
    ...
    const release: PluginRelease = {
      ...
      reviewNotes: blocked
        ...
        : isApproved
          ? 'Auto-approved by static analysis (sandboxed WASM)'
          : 'Queued for human review — JS plugins cannot self-approve',
    };
    ...
    return {
      ...
      scanResults,
      message: blocked
        ? 'Plugin rejected by static analysis'
        : isApproved
          ? 'Plugin approved and published'
          : 'Plugin queued for human review',
    };
  }
