...
//     • Meet  → resolves conflicts via H¹ monitoring of the contradiction complex
...
// same subject — NOT on pairwise phase interference derived from a content
// hash, which has no semantic meaning.
 ...
 * Mirrors HarmonicSheafWeaver's classifier so both forges agree.
 */
 ...
 * subgraph formed by 'contradicts' edges among `nodeIds` — the count of
 * *irreducible* contradiction cycles. Tree-like or disconnected contradiction
 * pairs contribute 0 (locally resolvable); each independent closed loop
 * contributes 1. Near-zero graph-Laplacian eigenvalues are NOT used for this
 * — they count connected components of the memory graph, not obstructions.
 */
function contradictionComplexBetti(nodeIds: Set<string>, edges: ERLEdge[]): number {
  let edgesUsed = 0;
...
/**
 * Number of connected components (b0 ≠ 0) induced by ALL edges among nodeIds.
 */
function contradictionConnectedComponents(nodeIds: Set<string>, edges: ERLEdge[]): number {
  const parent = new Map<string, string>();
  const find = (x: string): string => {
    const root = parent.get(x);
    if (root === undefined || root === x) return x;
  ...
  const components = new Set<string>();
  for (const id of nodeIds) if (parent.has(id)) components.add(find(id));
  let isolated = 0;
...
export class AetherForgeERL implements IMemoryLayer {
  ...
  weave(
    ...
    for (const existing of activeDomainNodes) {
      ...
      if (sim >= SUPERSESSION_THRESHOLD) {
        ...
        if (contraScore >= CONTRADICTION_DECISION_THRESHOLD) {
          ...
          const restrictionErr = Math.round(contraScore * 10000) / 10000;
    ...
    // Insert the node into the lattice store BEFORE join/meet (M43): both
    // operations look up this.storeData.nodes[nodeIdA] and return null/no-op
    // when the node is missing. Previously this insertion happened after, so
    // the 'Join/meet preserve global consistency' differentiator was dead in
    // the primary weave() path.
  ...
  /**
   * Contradiction detection via the lattice's contradiction complex.
   *
   ...
   *
   * The lattice Laplacian's near-zero eigenvalues are NOT used for betti1 —
   * they count connected components, not obstructions. Eigenvalues are still
   * computed for the spectralGap (algebraic connectivity / coherence signal).
   * Epistemic status still contributes: nodes upgraded to 'contradiction'
   * status participate in contradictionNodeIds.
   */
  detectContradictions(opts: { domain?: ERLDomain } = {}): ERLCohomologyResult {
    ...
    // Phase 4b: warm-started eigenmode computation
    ...
    // reports how far the lattice Laplacian sits from the H¹ band — a
    // coherence signal, not itself a contradiction count.
    const spectralGap =
  ...
  query(
    ...
    // in-memory only; the next real write (meet/verify/contradict/archive)
  ...
  consolidate(quenchThreshold = QUENCH_THRESHOLD): ERLConsolidationReport {
    ...
    const b0 = contradictionConnectedComponents(
  ...
  getStatus(): ERLStatus {
    ...
    // Contradiction-complex H¹ + cached spectral gap (cheap snapshot, no re-eigen)
    const betti1 = contradictionComplexBetti(
      new Set(active.map((n) => n.id)),
      this.storeData.edges
    );
    let spectralGap = 1.0;
  ...
  refreshEigenmodes(): ReturnType<AetherForgeERL['detectContradictions']> {
