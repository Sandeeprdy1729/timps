...
// Asserts the honest (M58) behavior: Fisher-Rao geodesic kNN graph,
// descriptive curvature statistics, and damped echo propagation.
...
describe('QERW', () => {
  describe('weave', () => {
    it('creates a node with a curvature statistic', () => {
  ...
  describe('query', () => {
    it('returns results sorted by geodesic distance', () => {
      ...
      // The database node must rank above unrelated nodes
      expect(r.results[0].node.content).toContain('database');
    });
