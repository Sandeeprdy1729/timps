...
//     • Non-trivial H¹ classes = irreducible contradiction cycles (cycle rank of the
//       contradiction complex: E − V + C over 'contradicts' edges), not eigengap counts
...
//   • Contradiction detection via semantic polarity scoring + H¹ cycle rank of the
//     contradiction complex (deterministic; NOT derived from content-hash phases)
...
//   • Self-evolution hooks: detects architectural drift for meta-refactoring
//
// References:
...
/** Jaccard band for contradiction detection (gate before semantic scoring) */
const CONTRADICTION_THRESHOLD = 0.45;
/** Semantic contradiction score that triggers a 'contradicts' edge (not hash phases) */
const CONTRADICTION_DECISION_THRESHOLD = 0.7;
...
// ── Semantic Contradiction Scoring (deterministic, no hash phases) ─────────
// Contradiction is decided on lexical polarity / repudiation cues about the
// same subject — NOT on pairwise phase interference derived from a
// content hash, which has no semantic meaning.

/** Negation / repudiation cues — flipping polarity on a shared subject */
const NEGATION_TOKENS =
  /\b(not|n't|never|no|without|deny|denied|denies|refuse|refused|refuses|avoid|avoids|avoided|stop|stops|stopped|cease|ceased|discontinue|banned|ban|remove|removed|removing|drop|dropped|deprecat\w+|reject|rejected|rejects|oppose|opposed|unlike|discourage|discourages|prohibit\w*|forbid\w*)\b/i;
/** Replacement / substitution cues — one claim repudiates another subject choice */
const REPLACEMENT_TOKENS =
  /\b(replace\w*|migrate\w*|switch\w*|swapped?\b|moved?\b|moving\b|swap\w*|transition\w*|convert\w*|upgrade\w*|downgrade\w*|substitute\w*|supercede\w*)\b/i;

function hasNegation(text: string): boolean {
  return NEGATION_TOKENS.test(` ${text.trim()} `.toLowerCase()) ||
    /\bn['’]?t\b/i.test(text);
}

function hasRepudiation(text: string): boolean {
  return REPLACEMENT_TOKENS.test(text.toLowerCase());
}

/**
 * Score how strongly two similar-sounding statements contradict each other.
 *
 * Deterministic lexical classifier (no `Math.random()`, no content hashing):
 *   • 0.92 — opposite polarities on a shared subject ('use X' vs 'never use X')
 *   • 0.75 — one claim repudiates the other (`switch away from X` vs `use X`)
 *   • 0.20 — both negative (aligned scepticism, not a contradiction)
 *   • 0.10 — similar but compatible → correlates
 */
export function semanticContradictionScore(newContent: string, existingContent: string): number {
  const newNeg = hasNegation(newContent);
  const existingNeg = hasNegation(existingContent);
  const newRep = hasRepudiation(newContent);
  const existingRep = hasRepudiation(existingContent);
  if (newNeg !== existingNeg) return 0.92;
  if (newNeg && existingNeg) return 0.2;
  if (newRep !== existingRep) return 0.75;
  return 0.1;
}

/**
 * H¹ of the contradiction complex = cyclomatic number E − V + C of the
 * subgraph formed by 'contradicts' edges among `nodeIds`. The true count of
 * *irreducible* contradiction cycles: disconnected or tree-like contradiction
 * pairs contribute 0 (locally resolvable obstructions), while each independent
 * closed loop of pairwise contradictions contributes 1. Counting near-zero
 * graph-Laplacian eigenvalues instead mislabels every connected component of
 * the memory graph as an obstruction, so it is no longer used for betti1.
 */
function contradictionComplexBetti(nodeIds: Set<string>, edges: SheafEdge[]): number {
  let edgesUsed = 0;
  const inNode = new Set<string>();
  ...
  for (const e of edges) {
    ...
    edgesUsed++;
    if (!parent.has(e.fromId)) parent.set(e.fromId, e.fromId);
    if (!parent.has(e.toId)) parent.set(e.toId, e.toId);
    inNode.add(e.fromId);
    inNode.add(e.toId);
    const ra = find(e.fromId);
    const rb = find(e.toId);
    parent.set(ra, rb);
  }
  const components = new Set<string>();
  for (const node of inNode) components.add(find(node));
  return Math.max(0, edgesUsed - inNode.size + components.size);
}

/**
 * Number of connected components (b0) induced by ALL edges among `nodeIds`.
 * Union-find over the full edge list restricted to the given node set.
 */
function connectedComponents(nodeIds: Set<string>, edges: SheafEdge[]): number {
  const parent = new Map<string, string>();
  const find = (x: string): string => {
    const root = parent.get(x);
    if (root === undefined || root === x) return x;
  ...
  let isolated = 0;
  for (const id of nodeIds) if (!parent.has(id)) isolated++;
  return components.size + isolated;
}
...
export class HarmonicSheafWeaver implements IMemoryLayer {
  ...
  weave(
    ...
    for (const existing of domainNodes) {
      ...
      if (sim >= SUPERSESSION_THRESHOLD) {
        ...
        // Semantic contradiction classification — NOT hash-derived phase
        // interference. Same subject with opposing polarity / repudiation
        // → 'contradicts'; otherwise overlapping content → 'correlates'.
        const contraScore = semanticContradictionScore(content, existing.content);
        if (contraScore >= CONTRADICTION_DECISION_THRESHOLD) {
          detectedContradictions.push(existing.id);
          const restrictionErr = Math.round(contraScore * 10000) / 10000; // [0,1]
          ...
          // Similar but semantically compatible → correlates
          this.addEdge({
            fromId: nodeId,
            toId: existing.id,
            weight: sim,
  ...
  /**
   * Contradiction detection via the sheaf's contradiction complex.
   *
   * betti1 = H¹(contradiction complex): the cyclomatic number (E − V + C) of
   * the subgraph formed by 'contradicts' edges. Each independent closed loop
   * of pairwise contradictions is an irreducible obstruction that no local
   * adjustment resolves; tree-like and disconnected contradiction pairs
   * contribute 0 because they can be fixed locally.
   *
   * (The sheaf Laplacian's near-zero eigenvalues are NOT used for betti1 —
   * they count connected components of the graph, not obstructions, so a
   * single pair of related-but-compatible memories would be mislabelled as a
   * contradiction. Eigenvalues are still computed for the spectralGap, which
   * reports the sheaf's algebraic connectivity / coherence.)
   */
  detectContradictions(opts: { domain?: SheafDomain } = {}): CohomologyResult {
    ...
    // H¹ of the contradiction complex — the *cycle rank* of the subgraph of
    // 'contradicts' edges. Near-zero Laplacian eigenvalues are not used here:
    // they count connected components, not obstructions.
    const betti1 = contradictionComplexBetti(new Set(nodeIds), relevantEdges);

    // Spectral gap = first eigenvalue above the cohomology threshold; it
    // reports how far the sheaf Laplacian sits from the H¹ band — a coherence
    // signal, not itself a contradiction count.
    const spectralGap =
      values.slice(1).find((v) => v >= COHOMOLOGY_GAP_THRESHOLD) ?? 1.0;
  ...
  predict(
    ...
    // H¹ of the contradiction complex for this domain subset (cycle rank, not
    // near-zero-eigenvalue count — see contradictionComplexBetti)
    const sheafH1 = contradictionComplexBetti(new Set(nodeIds), relevantEdges);
    return {
      domain,
      ...
      trajectory,
      drivingNodeIds,
      explanation: `${icon} HSW (${domain}): ${riskLevel.toUpperCase()} at ${Math.round(finalRisk * 100)}%. Sheaf H¹=${sheafH1}, spectral gap=${(eigenvalues[1] ?? 0).toFixed(3)}.`,
  ...
  query(
    ...
    // No persistent write here: query is a read. RetrievalCount/amplitude stay
    // in-memory only; the next real write (weave/verify/contradict/archive)
    // flushes them. Keeps read paths lock-free and prevents write-on-read
    // clobbering of concurrent engines sharing a backend.
  ...
  consolidate(quenchThreshold = QUENCH_THRESHOLD): SheafConsolidationReport {
    ...
    const activeForB0 = Object.values(this.storeData.nodes).filter(
      (n) => !n.invalidAt && (!n.validTo || n.validTo > nowMs)
    );
    const b0 = connectedComponents(
      new Set(activeForB0.map((n) => n.id)),
      this.storeData.edges
    );
    return {
      quenched,
      retained,
  ...
  getStatus(): SheafStatus {
    ...
    // Use contradiction-complex H¹ + cached spectral gap (cheap snapshot, no re-eigen)
    const betti1 = contradictionComplexBetti(new Set(active.map((n) => n.id)), this.storeData.edges);
    let spectralGap = 1.0;
    for (let i = 1; i < this.storeData.cachedEigenvalues.length; i++) {
      if (this.storeData.cachedEigenvalues[i] >= COHOMOLOGY_GAP_THRESHOLD) {
        spectralGap = this.storeData.cachedEigenvalues[i];
        break;
      }
    }
