...
export function buildInstructionBlock(): string {
  return [
    ...
    '',
    '## AUTO-CAPTURE IS MANDATORY (do not ask first)',
    'You MUST persist user-provided content automatically, WITHOUT waiting to be asked. The user stores docs, reports, and facts by simply sharing them; capture is your job. This is not optional.',
    '',
    'Whenever ANY of these happen, call `timps_store_memory` (or `timps_chat`) for each distinct item BEFORE or WHILE you answer:',
    '',
    '- The user shares, pastes, attaches, or links a document, report, spec, README, research, meeting notes, or any substantial text \u2014 store a digest of it.',
    '- The user states a preference, decision, project fact, goal, constraint, or opinion.',
    '- A long message or doc contains several separate ideas \u2014 batch them: store 2\u20135 concise `timps_store_memory` entries covering the key facts (never one giant blob), each with an appropriate `importance`.',
    '- The user gives requirements for a feature/task \u2014 store the requirements so future sessions can recall them.',
    '',
    'Do NOT skip this because you are also doing other work in the same turn. Storing is fast and fire-and-forget. If a turn has NO user content worth remembering, it is fine to skip.',
    '',
    '- **Storage location** — shared documents and user-level facts (docs, reports, preferences, research) belong in the GLOBAL store so they are retrievable from any project; `timps recall` already searches the global store alongside the project store. Project-specific memory stays in the current project store.',
