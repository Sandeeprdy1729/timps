...
import type { TimpsConfig, ProviderName, TrustLevel } from '../config/types.js';
...
export async function runSetupWizard(existing?: TimpsConfig): Promise<TimpsConfig> {
  ...
  const defaultProviderIndex = providerKeys.indexOf('ollama') + 1;
  const choice = await rl.question(t.prompt(`\n  Select [1-${providerKeys.length}] (default ${defaultProviderIndex}=ollama): `));
  const idx = Math.max(0, Math.min(providerKeys.length - 1, parseInt(choice || String(defaultProviderIndex)) - 1));
  ...
  const model = await rl.question(t.prompt(`  Model (default ${defaultModel}): `));
  config.defaultModel = model.trim() || defaultModel;

  if (['ollama', 'opencode', 'timps', 'timps-coder', 'hybrid'].includes(config.defaultProvider)) {
    const url = await rl.question(t.prompt('  Ollama URL (default localhost:11434): '));
    if (url.trim()) config.ollamaUrl = url.trim();
    console.log(t.dim('\n  Local model ideas: deepseek-r1 · qwen2.5-coder · codellama · llama3.1 · mistral'));
  }
